...
const SEAL = () => {
  ...
  const [sealCommType, setSealCommType] = useState("");
  ...
  const submitData = async () => {
    ...
    if (!sealType) {
      Alert.alert("Error", "Please select SEAL type");
      setSending(false);
      return;
    }
    ...
    try {
      const res = await fetch(
        "https://assign-meter-backend.onrender.com/api/sealassign",
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={{ marginTop: 15, marginBottom: 5 }}>SEAL Type</Text>
        ...
        >
          <Text>{sealType || "Select SEAL Type"}</Text>
          <Feather name="chevron-down" size={18} />
        </TouchableOpacity>
